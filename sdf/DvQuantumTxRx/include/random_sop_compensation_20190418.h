...
enum class operationModes { monitoringMode, actuationMode };
...
public:
	...
	void setQberTh(double qthresh) { qberTh = qthresh; e = (int)(qberTh * 100); };
	double getQberTh() { return qberTh; };

	void setNumberOfBitsToQBER(int n) { nb = n; numberOfBits = n; };

	void setIncControl(int incB) { incBits = incB; };
...
private:
	...
	bool actuate{ false };
	bool discard{ false };
	double epcDelayTimeSeconds{20e-6}; // in seconds
	...
	int receivedBits_toControl{ 0 };

	//###############Systen state Parameters########################################################################################
	operationModes opMode{ operationModes::monitoringMode };
	...
	void clearBuffer(vector <int>(&buff));
	void writeFile();
};
