	...
	else
	{
		...
		case signal_value_type::t_photon_mp_xy:
			for (auto k = 0; k < process; k++) {
				...
				double nThisDetector = ((pow(abs(inValueMP.path[path].x), 2) + pow(abs(inValueMP.path[path].y), 2)) / (PLANCK_CONSTANT*opticalFrequency));
				double nOtherDetector = ((pow(abs(inValueMP.path[otherPath].x), 2) + pow(abs(inValueMP.path[otherPath].y), 2)) / (PLANCK_CONSTANT*opticalFrequency));
				
				nThisDetector = (nThisDetector < 0.00001) ? 0.0 : nThisDetector;
				nOtherDetector = (nOtherDetector < 0.00001) ? 0.0 : nOtherDetector;

				double probabilityOfClick = nThisDetector*efficiency/2 + nOtherDetector*efficiency/2;
//				double probabilityOfClick = (1 - exp(-efficiency * nThisDetector)) + (1 - exp(-efficiency * nOtherDetector));
				
				double probabilityOfThisDetectorClick = nThisDetector*efficiency/2;
//				double probabilityOfThisDetectorClick = 1 - exp(-efficiency * nThisDetector);

				/*Add darkcounts*/
				probabilityOfThisDetectorClick = probabilityOfThisDetectorClick + probabilityDarkCount - (probabilityOfThisDetectorClick * probabilityDarkCount);

				double probabilityOfTheOtherDetectorClick = nOtherDetector / 2;

				if (probabilityOfClick > 1) 
					cout << "ERRO: single_photon_detector.cpp - probability of click greater than 1" << "\n";
				
				int number{ 0 };
				if ((k == 0) && (nThisDetector!=0)) {
					std::poisson_distribution<int> distribution(nThisDetector);
				...
				else
				{
					...
					inValueMP.path[otherPath].x = (probabilityOfTheOtherDetectorClick == 0) ? 0 : sqrt(probabilityOfTheOtherDetectorClick)*inValueMP.path[otherPath].x;
					inValueMP.path[otherPath].y = (probabilityOfTheOtherDetectorClick == 0) ? 0 : sqrt(probabilityOfTheOtherDetectorClick)*inValueMP.path[otherPath].y;
					probabilityOfClick = pow(abs(inValueMP.path[otherPath].x), 2) + pow(abs(inValueMP.path[otherPath].y), 2);
