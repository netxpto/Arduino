...
bool SOPRotator::runBlock(void) {
	...
	for (auto k = 0; k < process; k++) {
		...
		if (sopRotationType == sop_rotation_type::Stocastic) 
		{
			...
			double sigmaSquare = 2 * PI*delta_p*(inputSignals[0]->getSamplingPeriod());
			...
			/*alpha*/
			UniformRandomRealNumbers<> uniformeRandomNumber{ seed, 0, sqrt(standardev) };
			
			alpha[0] = uniformeRandomNumber();
			alpha[1] = uniformeRandomNumber();
			alpha[2] = uniformeRandomNumber();
