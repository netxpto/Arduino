...
void RandomSopCompensation::initialize(void) {
	...
	epcDelayBits = (int)(epcDelayTimeSeconds / (inputSignals[0]->getSymbolPeriod()));
	aux_epcBits = epcDelayBits ;
}
...
bool RandomSopCompensation::runBlock(void) {
	...
	if (firstTime) {
		...
		auto p = min(outputSignals[3]->space(), 1);
		for (auto n = 0; n < p; n++) {
			outputSignals[3]->bufferPut((t_binary)0);
		}

		auto l = min(outputSignals[5]->space(), 1);
		for (auto n = 0; n < l; n++) {
			outputSignals[5]->bufferPut((t_binary)0);
		}
	...
	if (process <= 0) {
		...
		myfile << "DoubleClick control = " << doubleClickControl << "\n";
		myfile << "NoClick control = " << nocControl << "\n";
		myfile << "Total number of received bits to control = " << receivedBits_toControl << "\n";
		myfile << "Number of received bits for QBER estimation = " << receivedBits << "\n";
		myfile << "Number of total received bits = " << n_aux << "\n";
	...
	for (auto k = 0; k < process; k++) {
		...
		switch (opMode)
		{
		case operationModes::monitoringMode:
			if (inputSignals[0]->ready() > 0) {
				...
				if (((n_control % incBits) == 0)) {
					qberValue = quantumBitErrorRate(signalValue);
					QBER = qberValue;
					if (((int)receivedBits % 50) == 0) // Trocar o receivedBits  >= 50 por 100
						reset = 1.0;
					discard = true;
				}
			...
			// Trocar o receivedBits  >= 20 por 100
			//if ((QBER > qberTh) && (receivedBits_toControl >= 100)) {
			if (QBER > qberTh){
				modeControl = false;
				opMode = operationModes::actuationMode;
				actuate = true;
				reset = 1.0;
		...
		case operationModes::actuationMode:
			rotFactor = 0.5;
			discard = true;
			if ((errors > 0) && (receivedBits < nBits)) {
				reset = 0.0;
				inputSignals[0]->bufferGet(&signalValue);
				if (epcDelay) { }
				else {
				...
				if ((receivedBits < numberOfBits) && ((firstPoint==true && secondPoint == true) || (firstPoint == false && secondPoint == false))) {
					inputSignals[0]->bufferGet(&signalValue);
					if(epcDelay){}
					else {
						qberValue = quantumBitErrorRate(signalValue);
					...
					if (actuate == true) {
						...
						case 0:
							/*	if (QBER <= qberTh) {
									...
									secondPoint = true;
									actuate = false;
									opMode = operationModes::monitoringMode;
									switchMode = true;
									if (window < 0)
										window = lengthWindow;
								}
						...
						case 1:
							if (QBER == 1)
							{
								...
								theta_aux = max(theta, phi);
								theta1 = theta_aux * 0.5 * 0.5;
								theta2 = theta_aux * 0.25 * 0.5;
								theta3 = 0.0;
							}
							reset = 1.0;
						...
						//			epcDelay = true;
							...
							//epcDelay = false; //APAGARRRRRRRRRR
							break;
	...
	if (opMode == operationModes::monitoringMode) {
		outputSignals[3]->bufferPut((t_binary)0);
		outputSignals[5]->bufferPut((t_binary)0);
	}
	else {
		outputSignals[3]->bufferPut((t_binary)1);
		outputSignals[5]->bufferPut((t_binary)1);
	}

	if (discard == false)
		outputSignals[4]->bufferPut((t_real)signalValue);
	else {
		discard = false;
		outputSignals[4]->bufferPut((t_real)5.0);
	}

	Qber_UB = QBER + 1 / sqrt(receivedBits) * z  * sqrt(QBER*(1 - QBER)) + 1 / (3 * receivedBits)*(2 * z * z * (1 / 2 - QBER) + (2 - QBER));
	outputSignals[6]->bufferPut((t_real)Qber_UB);
			

	return true;
}

void RandomSopCompensation::writeFile()
{
	n++;
	...
	myfile << "DoubleClick control = " << doubleClickControl << "\n";
	myfile << "Total number of received bits to control = " << receivedBits_toControl << "\n";
	myfile << "Number of received for QBER estimation bits = " << receivedBits << "\n";
	myfile << "Number of total received bits = " << n_aux << "\n\n";
...
void RandomSopCompensation::clearBuffer(vector <int>(&buff)) {
	int buff_size = (int)buff.size();
