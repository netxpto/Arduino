...
bool RandomSopCompensation::runBlock(void) {
	...
	if (firstTime) {
		...
		auto p = min(outputSignals[3]->space(), 16);
		for (auto n = 0; n < p; n++) {
	...
	for (auto k = 0; k < process; k++) {
		...
		case operationModes::monitoringMode:
			if (inputSignals[0]->ready() > 0) {
				...
				if (((n_aux % incBits) == 0)) {
					...
					QBER = qberValue;
					if (((int)receivedBits % 100) == 0)
						reset = 1.0;
				}
				n_aux++;
			}
			
			if ((QBER > qberTh) && (receivedBits >= 100)) {
				modeControl = false;
		...
		case operationModes::actuationMode:
			...
			if ((errors > 0) && (receivedBits <= nBits)) {
				...
				if ((receivedBits <= numberOfBits) && (firstPoint==true && secondPoint == true)) {
					inputSignals[0]->bufferGet(&signalValue);
					qberValue = quantumBitErrorRate(signalValue);
					QBER = qberValue;
					n_aux++;
