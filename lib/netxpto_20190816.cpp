...
//	bool alive = superBlockSystem.run();
	...
	if (!std::experimental::filesystem::is_directory(signalPath) || !std::experimental::filesystem::exists(signalPath)) {
		std::experimental::filesystem::create_directory(signalPath);
	}
	//Debug information
	clock_t start;
...
/*			case signal_value_type::t_message:
				...
				break;*/
			default:
				std::cerr << "ERRO: netxpto_20180815.cpp (SuperBlock)" << "\n";
				return false;
			}
		}
