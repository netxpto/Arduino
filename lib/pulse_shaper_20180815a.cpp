...
void square(vector<t_real> &impulseResponse, int impulseResponseLength, double pulseWidth, double samplingPeriod, double symbolPeriod) {
	...
	int high = (int) ceil(pulseWidth / samplingPeriod) - 1;
...
//	int pulseWidthLength = (int)ceil(pulseWidth * samplesPerSymbol);

	int pulseDelaySamples = (int)ceil(pulseDelay * samplesPerSymbol)-1;
	int high = (int)ceil(pulseWidth * samplesPerSymbol) - 1;
...
//	pulseWidthLength = pulseWidthLength + pulseDelaySamples;
	...
	for (int k = pulseDelaySamples; k < pulseDelaySamples+high; k++) {
		impulseResponse[k] = 1.0;
	};
}
	*/
