...
void square(vector<t_real> &impulseResponse, int impulseResponseLength, double pulseWidth, double samplingPeriod, double symbolPeriod);
void timeDelayedDeltaDirac(vector<t_real> &impulseResponse, int impulseResponseLength, double timeDelay, double samplingPeriod, double symbolPeriod);
...
void PulseShaper::initialize(void) {
	...
	switch (getFilterType()) {
		...
		case pulse_shapper_filter_type::Square:
			square(impulseResponse, impulseResponseLength, pulseWidth, samplingPeriod, symbolPeriod);
			break;
		case pulse_shapper_filter_type::TimeDelayedDeltaDirac:
			timeDelayedDeltaDirac(impulseResponse, impulseResponseLength, timeDelay, samplingPeriod, symbolPeriod);
			break;
	};
...
void square(vector<t_real> &impulseResponse, int impulseResponseLength, double pulseWidth, double samplingPeriod, double symbolPeriod) {
	...
	int high = (int) round(pulseWidth / samplingPeriod) - 1;

	for (int k = 0; k <= high; k++) {
		impulseResponse[k] = 1.0;
	};

	for (int k = high+1; k < impulseResponseLength; k++) {
...
void timeDelayedDeltaDirac(vector<t_real> &impulseResponse, int impulseResponseLength, double timeDelay, double samplingPeriod, double symbolPeriod) {
	...
	impulseResponse[(size_t) round(timeDelay/samplingPeriod)] = 1.0;
};
