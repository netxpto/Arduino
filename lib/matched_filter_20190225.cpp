...
void rootRaisedCosine1(vector<t_real> &impulseResponse, int impulseResponseLength, double rollOffFactor, double samplingPeriod, double symbolPeriod, bool passiveFilterMode);
...
void MatchedFilter::initialize(void) {
	...
	rootRaisedCosine1(impulseResponse, blockSize, rollOffFactor, samplingPeriod, symbolPeriod, true);
...
bool MatchedFilter::runBlock(void) {
	...
	if (space > 0 && space != outputSignals[0]->getBufferLength()) return false;
	
	if (count != 0)
	{
		int process = min(count, space);
	...
	for (int i = 0; i < ready; i++)
	{
		...
		if (aux == blockSize)
		{
			...
			{
				outVector.erase(outVector.begin() + outVector.size() - blockSize / 2, outVector.begin() + outVector.size());
			...
			{
				outVector.erase(outVector.begin(), outVector.begin() + blockSize / 2);
				outVector.erase(outVector.begin() + outVector.size() - blockSize / 2, outVector.begin() + outVector.size());
				count = outVector.size();
