...
void calculateHeader(t_integer bPosition, t_binary &bitValue, frame_header_mode framerMode, t_integer headerLen, t_integer headerSigLen, t_integer fCount, t_integer &toAdd);
...
bool Framer::runBlock(void)
{
	...
	int nFrames = (int)floor((t_real)ready / ((t_real)(frameLength - headerLength)));   // one bit in the frame needs two random bits (one bit value and one basis value)
